...
- **Click-to-skip & choice clicks** -- single scene-level `pointerdown` handler registered upfront (not delayed in `showChoices`) so clicking the screen during the typewriter skips to end, and clicking choice buttons after typing selects them
- **Rescue SFX** -- `audio.playItemPickup()` plays on rescue confirmation
...
- **Click-to-skip not working during typewriter** -- `pointerdown` handler was only registered inside `showChoices()` (after typing completed), so clicking the screen while typing did nothing. Fixed by replacing delayed `_clickHandler` with a single `clickHandler` registered upfront alongside keyboard handlers that handles both skip (while typing) and choice selection (after typing).
- **Regeneration Boots counter reset per floor** -- `this.rocksBrokenThisRun = 0` inside `rebuildFloor()` wiped the counter every floor descent, making the "1 stamina per 5 rocks" effect nearly impossible to trigger. Fixed by removing the reset so the counter accumulates across the whole expedition.
- **Boots/lantern usage shows 0 after crafting** -- `consumeEquipmentRun()` incremented `itemRuns[id]` to 5 but never `delete`d the key when the last copy was consumed; crafting a new pair inherited the stale counter. Fixed by `delete this.itemRuns[itemId]` in the depletion branch and resetting `itemRuns[recipe.result] = 0` on first-time craft (matching the pickaxe pattern).

## ✅ Regeneration Boots Popup & SFX (July 2026)
- **"+1 stamina" notification** -- `showBootsRegenPopup()` displays a stacked popup (boots icon + cyan `"+1 stamina"` text) using the same visuals as `queueObtainPopup`, auto-fades after 1.5s
- **Audible regen tick** -- `audio.playRegen()` added as a short rising sine sweep (660→880 Hz, 0.15s) for clear but unobtrusive feedback

## ✅ Boss Drop Pickaxe Variants (July 2026)
- **5 boss-crafted pickaxes** -- each biome boss drops a unique material used to craft a specialized pickaxe:
  - **Verdant Pick** (tier 5, power 3) -- Forest Gem + silver + crystal -- 25% stamina refund on mine
  - **Stoneheart Pick** (tier 6, power 4) -- Cave Heart + gold + crystal -- 20% double ore drops
  - **Permafrost Pick** (tier 7, power 5) -- Ice Shard + gold + crystal -- -1 stamina cost per mine
  - **Magma-Pick** (tier 8, power 5) -- Magma Core + gold + crystal -- 15% bomb blast proc on mine
  - **Void Pick** (tier 9, power 6) -- Void Essence + gold + crystal -- auto-collect mined items
- **Recipe discovery on boss defeat** -- each boss's pickaxe recipe unlocks immediately when you defeat that biome boss (alongside existing stamina potion recipe)
- **Balanced power curve** -- boss pickaxes spread across silver/gold/crystal recipe costs to bridge silver->endgame progression